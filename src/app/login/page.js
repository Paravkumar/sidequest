...
function LoginContent() {
  ...
  const [formData, setFormData] = useState({ name: "", email: "", password: "", phone: "+91 " });
          ...
          {isRegistering && (
            <>
                ...
                <div>
                    ...
                    <input type="text" required value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="mt-1 w-full rounded-lg bg-slate-800 border border-white/10 p-3 text-white outline-none focus:border-violet-500 transition" placeholder="+91 9876543210" />
                </div>
            </>
          )}
